...
function getNextFriday(date) {
  ...
  const friday = (5 - day + 7) % 7;
  const result = new Date(date);
  result.setDate(date.getDate() + friday + (friday === 0 ? 7 : 0));
  return result;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(value / (1000 * 60 * 60 * 24)) + 1;
}
...
function formatDate(date) {
  const per = new Date(date);
  const year = per.getUTCFullYear();
  const month = per.getUTCMonth() + 1;
  const day = per.getUTCDate();
  let hour = per.getUTCHours();
  const minute = per.getUTCMinutes().toString().padStart(2, '0');
  const second = per.getUTCSeconds().toString().padStart(2, '0');
  const value = hour >= 12 ? 'PM' : 'AM';
  hour = hour % 12 || 12;
  return `${month}/${day}/${year}, ${hour}:${minute}:${second} ${value}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    const numberInWeek = date.getDay();
    if (numberInWeek === 0 || numberInWeek === 6) {
      value += 1;
    }
  }
  return value;
...
function getQuarter(date) {
  const result = [1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4];
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  function descriptoin(date) {
    const days = String(date.getDate()).padStart(2, '0');
    const mon = String(date.getMonth() + 1).padStart(2, '0');
    const years = date.getFullYear();
    return `${days}-${mon}-${years}`;
  }
  while (value <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (value <= end) {
        array.push(descriptoin(value));
      }
...
function isLeapYear(date) {
  ...
  return new Date(value, 1, 29).getDate() === 29;
}
